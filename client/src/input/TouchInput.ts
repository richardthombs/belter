...
export class TouchInput {
	...
	private processTouchPosition(clientX: number, clientY: number): void {
		...
		// from accidental diagonal drags. Tie-break (absDx === absDy, 45°):
		// thrust wins (>= vs >) — forward/back takes priority over rotation.
		const absDx = Math.abs(rawDx);
		const absDy = Math.abs(rawDy);
		const thrust =
			absDy >= absDx ? this.applyDeadZoneAndNormalise(-rawDy) : 0;
		const torque =
