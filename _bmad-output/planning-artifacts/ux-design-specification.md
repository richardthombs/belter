...
- **Sparse-space orientation → subtle background reference** — when nearby objects are absent, provide a very faint motion-reference backdrop so travel direction remains readable without competing with HUD or world entities. Current selected default: **sector-aligned grid**. Starfield remains non-default for explicit evaluation/debug overrides.
