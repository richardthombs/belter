...
- [ ] Task 1 — Extend asteroid domain model and persistence for dynamic simulation (AC: 1, 2, 3)
  - [ ] Update `server/BelterLife.Shared/Entities/Asteroid.cs` with drift/collision state fields (`VelocityX`, `VelocityY`, `IsDestroyed`) using existing project naming conventions.
  - [ ] Add an EF migration in `server/BelterLife.Simulation/Migrations/` for new asteroid columns; preserve existing snake_case conventions and defaults for backward compatibility.
  - [ ] Ensure migration supports existing databases safely (no runtime data fixups in request paths; migration-only policy).

- [ ] Task 2 — Seed initial asteroid drift in sector generation (AC: 1)
  - [ ] Update `server/BelterLife.Simulation/Entities/SectorGenerator.cs` so generated asteroids receive small initial velocity vectors.
  - [ ] Keep all positions in int64 millimetres and velocities in float mm/s.
  - [ ] Keep generation bounded by canonical `RegionBounds` constants; do not hardcode alternate sector sizes.

- [ ] Task 3 — Implement asteroid drift integration in physics layer (AC: 1)
  - [ ] Add asteroid-drift update logic in `server/BelterLife.Simulation/Physics/PhysicsEngine.cs` (or a dedicated method used by asteroid simulation orchestration).
  - [ ] Use int64 position integration pattern from Story 2.0 (`(long)(velocity * deltaSeconds)`), avoiding float position fields.

- [ ] Task 4 — Implement asteroid collision and fragmentation behavior (AC: 2, 3)
  - [ ] Implement/extend collision resolution in `server/BelterLife.Simulation/Physics/CollisionResolver.cs` for asteroid↔asteroid impacts.
  - [ ] Apply deterministic/controlled momentum response and bounded fragment generation.
  - [ ] Mark destroyed parents and ensure they are excluded from active world snapshots.
  - [ ] Create fragment entities with valid sector association and trajectories; persist in same simulation update transaction.

- [ ] Task 5 — Add `AsteroidManager` orchestration and wire into tick loop (AC: 1, 2, 3, 4)
  - [ ] Create/extend `server/BelterLife.Simulation/Entities/AsteroidManager.cs` to coordinate drift + collision + fragment persistence.
  - [ ] Register required services in `server/BelterLife.Simulation/Program.cs` following existing DI style.
  - [ ] Integrate asteroid manager invocation into `server/BelterLife.Simulation/Physics/SimulationLoop.cs` before world snapshot broadcast.
  - [ ] Ensure only non-destroyed asteroids are broadcast.

- [ ] Task 6 — Update contracts and client type compatibility (AC: 4)
  - [ ] Update shared hub contract(s) (for example `server/BelterLife.Shared/Contracts/Hubs/AsteroidSnapshot.cs`) if additional asteroid state is needed for rendering.
  - [ ] Mirror any contract changes in `client/src/types/index.ts` without changing JSON casing conventions.
  - [ ] Keep asteroid position values as int64-origin server fields represented as JS `number` on the client.

- [ ] Task 7 — Add/adjust tests for drift, collision response, and fragmentation (AC: 1, 2, 3, 4)
  - [ ] Add physics-level unit tests in `server/BelterLife.Simulation.Tests/Physics/` for asteroid drift integration and collision outcomes.
  - [ ] Add simulation-loop tests in `server/BelterLife.Simulation.Tests/Physics/SimulationLoopTests.cs` (or adjacent test files) validating server-authoritative asteroid updates and snapshot filtering.
  - [ ] Add migration/schema regression checks where appropriate for new asteroid fields.

- [ ] Task 8 — Validate build/test gates
  - [ ] Run `dotnet build server/BelterLife.slnx`.
  - [ ] Run targeted server tests for new/changed simulation files, then run broader `dotnet test server/BelterLife.slnx`.
  - [ ] Run `npm run build` in `client/` if shared contracts/types changed.
